...
First, you need an image. There's a dedicated folder within the project for this: `Electron-app\character_art\Characters`. Place all the character images you need here.
...
Open "Command Prompt" and type: `cd {the location of the folder}` — put the location of the folder where you placed it after the `cd` (for example, `cd D:\Github\Virtual_video_avatar\Electron-app`).
...
Once you're in the Electron-app folder, type: `npm start`. This starts the application. This comes with a default character that I use myself, along with a few settings:
...
- **"Toggle rotation marker"**: Toggles a marker that shows where the origin of the head is. (To change this position — which you'll probably need to do — you need to find a file called `style.css` in this folder: `D:\Github\Virtual_video_avatar\Electron-app\css`. In it, you'll find:
...
5. Place both images in the `Virtual_video_avatar\Electron-app\character_art\Characters` folder